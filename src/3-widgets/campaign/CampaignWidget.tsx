...
export function CampaignWidget({ isMobile }: CampaignWidgetProps) {
  ...
  useEffect(() => {
    ...
    setSelectedColor(PIXEL_PALETTE[selectedPixel % PIXEL_PALETTE.length]);
  }, [selectedPixel]);

  const availablePixelItems = HEART_PIXELS.filter((pixel) => {
  ...
  const availablePixels = availablePixelItems.length;

  const soldCount = HEART_PIXELS.length - availablePixels;

  const progress = (soldCount / HEART_PIXELS.length) * 100;
  ...
  const formattedSoldCount = soldCount.toLocaleString(numberLocale);

  const formattedAvailablePixels = availablePixels.toLocaleString(numberLocale);

  const isDonationDialogOpen = selectedPixel !== null || successPixel !== null;
  ...
  const chooseRandomPixel = () => {
    const available = HEART_PIXELS.filter((pixel) => {
      ...
      return (
        !isInitiallySold(pixel.id) && purchasedPixels[pixelNumber] === undefined
      );
    });
          ...
          {successPixel !== null ? (
            ...
            selectedPixel !== null && (
              <DonationFormContent
                ...
                onShowMessage={() => setShowMessage(true)}
